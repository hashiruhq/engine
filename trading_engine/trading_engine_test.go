...
func TestTradingEngineCreation(t *testing.T) {
	Convey("Given an empty trading engine", t, func() {
		...
		Convey("I should be able to process a new order", func() {
			tradingEngine.Process(NewOrder("TEST_1", uint64(100000000), uint64(12000000000), 1, 1, 1))
			tradingEngine.Process(NewOrder("TEST_2", uint64(110000000), uint64(12000000000), 2, 1, 1))
			trades := tradingEngine.Process(NewOrder("TEST_3", uint64(90000000), uint64(12000000000), 2, 1, 1))
